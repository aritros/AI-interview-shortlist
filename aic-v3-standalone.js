...
const REJ = ()=>C.filter(c=>c.status==='rej');
...
function verdictFor(c){
  ...
  if (c.score>=4.3) return {cls:'strong', lbl:'Clear advance', txt:`Drove the case end to end — ${s} against a ${avg} batch average, at or above your bar on every assessed criterion${skip}.`};
  if (c.score>=4)   return {cls:'strong', lbl:'Advance', txt:`A strong pass — ${s} vs the ${avg} batch average. Verify the flagged gap below, then move.`};
  if (c.score>=3.4) return {cls:'mod', lbl:'Second opinion', txt:`A middle-of-the-pack pass — ${s} vs the ${avg} batch average, with real gaps below. Better shared with your HM than advanced outright.`};
  if (c.score>=2.8) return {cls:'low', lbl:'Below bar', txt:`Below your bar at ${s} — consistent gaps across the weighted criteria this round.`};
  return {cls:'low', lbl:'Well below bar', txt:`Well below the bar at ${s} — did not engage with the core of the case.`};
}

/* ===== state ===== */
let tab='fresh', bucket=1, aiF=null, noteI=null, drawerI=null, openCmps=new Set();
const selected = new Set();
const BUCKETS = [
  {t:'All interviews', d:'Interviews completed', cls:'c-all', f:()=>true},
  {t:'Ready to advance', d:'Score 4.0+', cls:'c-adv', f:c=>c.score>=4&&c.proctor},
  {t:'Send for HM review', d:'Good scores, may need review', cls:'c-hm', f:c=>(c.score>=3.4&&c.score<4)||(c.score>=4&&!c.proctor)},
  {t:'Proctoring flags', d:'Suspicious activity flagged', cls:'c-flag', noAvg:true, f:c=>!c.proctor},
  {t:'Below your bar', d:'Did not meet the bar this round', cls:'c-bar', f:c=>c.score<3.4}
];
const FILTERS = {
...
function render(){
  ...
  $('actedCount').textContent = HMS().length + ADV().length + REJ().length;
...
function renderBuckets(){
  ...
  $('buckets').innerHTML = BUCKETS.map((b,j)=>{
    ...
    return `${j?'<div class="bkt-sep"></div>':''}<div class="bkt ${b.cls}${bucket===j&&!aiF?' sel':''}" data-bkt="${j}">
...
function renderFresh(){
  ...
  $('freshHead').innerHTML = `<div class="cb${allSel?' checked':''}" id="cbAll" title="Select all"></div>
    <div class="fh-cand">Candidates ${selected.size?`<span class="count-tag sel">${selected.size} selected</span><button class="sel-lnk red" id="selNone">Clear selection</button>`:`<span class="count-tag">${arr.length} of ${fresh().length}</span>`}${aiF?`<span class="fchip"><span class="lbl">${aiF.label}</span><span class="material-icons-round" id="clearF" title="Clear filter">close</span></span>`:''}</div>
    <div>AI Interview score</div><div class="fh-comp">Core competencies<i class="info-ic" tabindex="0">i<span class="tip">Hiring criteria met for this position</span></i></div>`;
  $('freshRows').innerHTML = arr.map(c=>`
    <div class="frow${c.i===drawerI?' sel':''}" data-i="${c.i}">
      ...
      <div class="who"><div class="nm">${c.name}</div><div class="rl">${c.role} · ${c.company}</div></div>
      <div class="sc ${c.score<3.4?'low':''}">${c.score.toFixed(1)}<small>/5</small></div>
      <div class="comp-cell">${c.comps.map(k=>`<span class="cchip">${k}</span>`).join('')}</div>
      <div class="row-acts">
...
function renderActed(){
  ...
  const rejs = REJ();
  $('rejCount').textContent = rejs.length;
  $('rejRows').innerHTML = rejs.map(c=>`
    <div class="lrow${c.i===drawerI?' sel':''}" data-i="${c.i}">
      <div class="avatar ${c.col}">${c.init}</div>
      <div class="who"><div class="nm">${c.name}</div><div class="rl">${c.role} · ${c.company}</div></div>
      <div class="sc">${c.score.toFixed(1)}<small>/5</small></div>
      <span class="status-chip rej"><span class="material-icons-round">block</span>Rejected</span>
      <span class="when">Rejected ${c.when}</span>
      <div class="row-acts"></div>
      ${noteBtn(c)}
    </div>`).join('') || `<div class="empty"><span class="material-icons-round">block</span>No rejected candidates.</div>`;
}
/* ===== detail drawer ===== */
...
function renderDrawer(){
  ...
  if (c.status==='rej') st = `<span class="status-chip rej"><span class="material-icons-round">block</span>Rejected · ${c.when}</span>`;
  const share = c.status ? '' : `<button class="ract hm" data-hm="${c.i}">Share with HM</button>`;
  const advB = (c.status==='adv'||c.status==='rej') ? '' : `<button class="ract gy" data-adv="${c.i}">Advance stage<span class="material-icons-round">expand_more</span></button>`;
  ...
  <div class="dw-ai">
    <div class="ai-row">
      <div class="ai-main">
        ...
        <div class="vh2"><span class="fit ${v.cls}">${v.lbl}</span><span>Ranked #${rank} of ${C.length} interviewed</span></div>
    ...
    <div class="pl-row">${plus.slice(0,3).map(x=>`<span class="pc plus">+ ${x}</span>`).join('')}${watch.map(x=>`<span class="pc watch">– ${x}</span>`).join('')}</div>
  ...
  <div class="dw-sec"><div class="hlp-h">Hiring criteria evaluation<span class="lg">| marks batch avg</span></div>
  <div style="margin-top:10px">${blocks}</div></div>
  <div class="dw-ask-wrap"><div class="ask-panel">
    <div style="font-size:13px;font-weight:600;color:var(--grey-color-90,#1a212e);margin-bottom:8px">Ask AI about ${c.first}</div>
...
function runAsk(q){
  const a = $('askA');
  if (!a) return;
  a.innerHTML = `<b style="color:var(--grey-color-90,#1a212e)">${q}</b><div style="margin-top:5px">${askAnswer(C[drawerI], q)}</div>`;
  a.classList.add('on');
}
...
document.addEventListener('click', e=>{
  ...
  if (bb){
    ...
    else if (bb.dataset.bulk==='rej'){
      const ids = [...selected];
      ids.forEach(i=>{ C[i].status='rej'; C[i].when='Just now'; });
      toast(`${ids.length===1?C[ids[0]].first:ids.length+' candidates'} rejected`);
      selected.clear(); render();
    }
    else {
      const ids = [...selected];
      ids.forEach(i=>{ C[i].status='hm'; C[i].when='Just now'; });
      toast(`${ids.length===1?C[ids[0]].first:ids.length+' candidates'} shared with your hiring manager for approval`);
      selected.clear(); render();
    }
    return;
  }
  ...
  if (hm){
    ...
    toast(`${c.first} shared with your hiring manager for approval`);
    render(); return;
  }
...
document.addEventListener('keydown', e=>{ if(e.key==='Escape'){ hideStageMenu(); closeNotes(); closeDrawer(); $('fltFab').classList.remove('open'); closeReel(); } });
