...
/* HM view: the recruiter's share — which candidates, her notes, and the HM's verdicts */
...
C.forEach(c=>{ c.shared = SHARED.has(c.i); c.note = SHARED.get(c.i) || null; c.verdict = null; });
const COMP_DEFS = [
...
Object.assign(FILTERS, {
  await: {k:'await', label:'Awaiting your verdict', title:'Awaiting your verdict', f:c=>!c.verdict,
          why:'Candidates Sarah shared that you haven\u2019t returned a verdict on yet, strongest first.'},
  done:  {k:'done', label:'Reviewed by you', title:'Reviewed by you', f:c=>!!c.verdict,
          why:'Everything you\u2019ve already returned a verdict on \u2014 thumbs up, discuss or thumbs down. Sarah sees each one as a status chip on her board.'}
});
const SUGS = ['await','done','skills','bar4','clean'];
const CAP = 10;
let cap = null;
const REFINES = {};
const NEXTS = {};
...
function syncChips(force){
  const sig = (cap||'') + '|' + (aiF ? aiF.baseK+':'+aiF.label : '') + '|' + live().length + '|' + C.filter(c=>c.shared && c.verdict).map(c=>c.i+c.verdict).join(',');
...
const live = ()=>C.filter(c=>c.shared).sort((a,b)=>b.score-a.score);
...
function renderList(arr){
  ...
  $('viewTitle').textContent = aiF ? (aiF.title || aiTitleFor(aiF.label)) : 'Shared with you';
  syncChips();
  ...
  $('cards').innerHTML = shown.map((c,n)=>`
    <article class="ccard${c.i===selI?' sel':''}" data-i="${c.i}" tabindex="0">
      ...
      ${barsHtml(c)}
      <div class="cc-state">
        ${c.verdict?`<span class="cst vd ${c.verdict}" title="Your verdict \u2014 sent to ${SHARER.name}"><span class="material-icons-round">${VICON[c.verdict]}</span>${VLBL[c.verdict]}</span>`:''}
      ...
      <div class="cc-acts">
        <button class="ca adv${c.verdict==='up'?' on':''}" data-verdict="up" data-ci="${c.i}"><span class="material-icons-round">thumb_up</span>Thumbs up</button>
        <button class="ca hm${c.verdict==='discuss'?' on':''}" data-verdict="discuss" data-ci="${c.i}"><span class="material-icons-round">forum</span>Discuss</button>
        <button class="ca hm${c.verdict==='down'?' on':''}" data-verdict="down" data-ci="${c.i}"><span class="material-icons-round">thumb_down</span>Thumbs down</button>
      </div>
...
function renderRail(){
  ...
  $('rail').innerHTML = `
    ...
    <div class="rail-acts" id="railActs">
      <button class="ract adv${c.verdict==='up'?' on':''}" data-verdict="up" data-ci="${c.i}" title="Thumbs up"><span class="material-icons-round">thumb_up</span><span class="vl">Thumbs up</span></button>
      <button class="ract hm${c.verdict==='discuss'?' on':''}" data-verdict="discuss" data-ci="${c.i}" title="Discuss"><span class="material-icons-round">forum</span><span class="vl">Discuss</span></button>
      <button class="ract hm${c.verdict==='down'?' on':''}" data-verdict="down" data-ci="${c.i}" title="Thumbs down"><span class="material-icons-round">thumb_down</span><span class="vl">Thumbs down</span></button>
    </div>`;
  queueClamp();
}
...
const VLBL = {up:'Thumbs up', discuss:'Discuss', down:'Thumbs down'}, VICON = {up:'thumb_up', discuss:'forum', down:'thumb_down'};
function setVerdict(ids, kind){
  if (!ids.length) return;
  const snap = ids.map(i=>({i, prev:C[i].verdict}));
  const clear = ids.every(i=>C[i].verdict===kind);
  ids.forEach(i=>{ C[i].verdict = clear ? null : kind; });
  picked.clear();
  undo = ()=>{ snap.forEach(s=>{ C[s.i].verdict = s.prev; }); undo=null; render(); };
  const who = ids.length===1 ? C[ids[0]].name : `${ids.length} candidates`;
  toast(clear ? `Verdict on ${who} withdrawn.` : `${who} \u2014 ${VLBL[kind].toLowerCase()} sent to ${SHARER.name}.`);
  render();
}
...
document.addEventListener('click', e=>{
  ...
  if (vb){ e.stopPropagation(); setVerdict(vb.dataset.ci!=null ? [+vb.dataset.ci] : [...picked], vb.dataset.verdict); return; }
