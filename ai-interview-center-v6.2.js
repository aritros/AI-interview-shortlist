...
const PARAMS = {
  ps:['Problem solving \u2265 3.8','Criterion weight 30%','Overall score ignored'],
  comm:['Communication \u2265 3.8','Criterion weight 20%','Transcript evidence weighted'],
  allTests:['All 13 coding tests passed','No partial credit'],
  craft:['Design case submitted','Craft scored against the bar'],
  _free:['Semantic match on transcripts','Overall score \u2265 3.4','All 20 interviews read']
};
...
function runFilter(next){
  ...
  $('aiWhy').hidden = true;
  $('fltZone').innerHTML = `<div class="flt-just"><svg class="spark" width="15" height="15" aria-hidden="true"><use href="#aiSparkle"></use></svg><span class="jt step" id="aiStep">${STEPS[0]}\u2026</span></div>`;
  ...
  runFilter._t = setTimeout(()=>{
    ...
    render();
    const n = shownOf(visible()).length;
    $('fltZone').innerHTML = `<div class="flt-just"><svg class="spark" width="15" height="15" aria-hidden="true"><use href="#aiSparkle"></use></svg><span class="jt">${aiF.why || aiWhyFor(aiF.label, n, live().length)} <b>Parameters applied:</b> ${(PARAMS[aiF.k]||PARAMS._free).join(' \u00b7 ')}.</span><button class="oct-btn" id="clearF">Reset</button></div>`;
...
function renderList(arr){
  ...
  if (aiF){ why.hidden = true; why.innerHTML = ''; }
...
document.addEventListener('click', e=>{
  ...
  if (e.target.closest('#clearF') || e.target.closest('#clearF2')){ stopFilter(); aiF=null; $('aiIn').value=''; setTyping(false); $('fltZone').innerHTML = DEFAULT_CHIPS; render(); return; }
  const sug = e.target.closest('[data-q]');
  if (sug){ const f = FILTERS[sug.dataset.q]; $('aiIn').value = f.label; setTyping(false); runFilter(f); return; }
...
function submitFilter(){
  ...
  setTyping(false);
  if (!q){ stopFilter(); aiF=null; $('fltZone').innerHTML = DEFAULT_CHIPS; render(); return; }
...
$('aiIn').addEventListener('input', e=>setTyping(true));
