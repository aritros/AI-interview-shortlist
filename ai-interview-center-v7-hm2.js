...
/* HM view: the recruiter's share — which candidates and her per-candidate notes */
const SHARER = {name:'Sarah Johnson', init:'SJ', when:'Tue, Sep 1'};
const SHARED = new Map([[0,'Strongest of the batch — skipped the case, but everything else is exceptional.'],[1,'My pick to move first. Clean session, nothing flagged.'],[2,null],[4,'Strong communicator; the failed tests are worth a look before onsite.'],[5,null],[7,null]]);
C.forEach(c=>{ c.shared = SHARED.has(c.i); c.note = SHARED.get(c.i) || null; });
const COMP_DEFS = [
...
function verdictFor(c){
  ...
  if (c.score>=3.4) return {cls:'mod', tag:'Mixed', lbl:'Second opinion', txt:`A middle-of-the-pack pass — ${s} vs the ${BATCH_AVG} batch average, with real gaps below. Worth a second opinion before deciding.`};
...
const FILTERS = {
  ...
  skills2:  {k:'skills2', label:'Excelled in design case', title:'Excelled in design case', f:c=>!c.caseNA,
             why:'Everyone who submitted the design case, so craft could actually be assessed.'}
};
const INT_SKILLS = {
...
const INT_SECOND = {
  '360 Interview': {label:'Excelled in design case', f:c=>!c.caseNA,
    why:'Everyone who submitted the design case, so craft could actually be assessed. Candidates who skipped it carry no craft signal.'},
  'Coding round': {label:'Passed every test case', f:c=>c.passed===c.total,
    why:'All test cases green \u2014 the round\u2019s own pass bar, independent of the weighted score.'},
  'Role fit': {label:'Handled pushback well', f:c=>c.comm>=3.6 && COMP_DEFS.find(d=>d.k==='collab').get(c)>=3.4,
    why:'Read from the moments they were challenged \u2014 communication at 3.6+ with collaboration at 3.4+.'},
  'Case study': {label:'Explored more than one direction', f:c=>!c.caseNA && c.ps>=3.6,
    why:'Submitted the case and showed alternative directions before committing \u2014 problem solving at 3.6+.'},
  'Communication': {label:'Explained without being asked twice', f:c=>c.comm>=3.9,
    why:'Communication at 3.9+ \u2014 the walkthrough needed no follow-up questions to land.'}
};
function applyInt(name){
  ...
  const s2 = INT_SECOND[name] || INT_SECOND['360 Interview'];
  Object.assign(FILTERS.skills2, {label:s2.label, title:s2.label, f:s2.f, why:s2.why});
  if (aiF && (aiF.baseK==='skills' || aiF.baseK==='skills2')) aiF = buildF(aiF.baseK, []);
  syncChips._s = null;
  render();
}
const SUGS = ['adv','skills','skills2','bar4'];
const CAP = 1;
let cap = 1;
const REFINES = {};
const NEXTS = {};
...
let pendF = null;
function chipsHtml(){
  const pool = live(), af = aiF || pendF;
  const out = [];
  const n = f => pool.filter(f).length;
  if (af && af.baseK === '_free') out.push(chipOn(af.label, 'base', n(af.f)));
  out.push(cap
    ? chipOn('Shared with you', 'cap', pool.length)
    : `<button class="sug" data-cap="1">Shared with you<span class="sug-n">${C.filter(c=>c.shared && !c.status).length}</span></button>`);
  SUGS.forEach(k=>out.push(af && af.baseK === k
  ...
  if (cap || af) out.push(`<button class="sug-clear" id="clearAll">Clear all</button>`);
  return out.join('');
}
...
const PH = [
  ...
  'who would be strongest working with my team',
  'anyone I should be worried about before onsite',
  'the clearest communicator of the ones Sarah shared',
  'scoring 4.0+ with clean proctoring',
  'who thought about users before jumping to solutions'
];
let phT = [], phI = 0;
...
const live = ()=>C.filter(c=>!c.status && (cap ? c.shared : true)).sort((a,b)=>b.score-a.score);
...
function clampRail(){
  const rail = $('rail');
  if (rail){
    if (selI==null) rail.style.maxHeight='';
    else rail.style.maxHeight = Math.max(220, Math.round(innerHeight - rail.getBoundingClientRect().top - 20)) + 'px';
  }
  const cp = $('cmpPanel');
  if (cp){
    const col = cp.parentElement;
    if (!cp.closest('.split').classList.contains('has-cmp')) cp.style.height='';
    else {
      /* cap against the sticky offset, never the live rect: in expanded mode the column
         cannot stick (its container is no taller than it is), so a rect-based height
         grows on every scroll and drags the page with it */
      const stick = parseFloat(getComputedStyle(col).top) || 104;
      const h = Math.min(innerHeight - stick - 20, innerHeight - col.getBoundingClientRect().top - 20);
      cp.style.height = Math.max(420, Math.round(h)) + 'px';
    }
  }
}
let clampQ = false;
...
$('rail').addEventListener('scroll', e=>{ const s = e.target.closest('.rail-scroll'); if (s) s.classList.toggle('scrolled', s.scrollTop > 2); }, {passive:true, capture:true});
...
function stopFilter(){ clearTimeout(runFilter._t); clearInterval(runFilter._i); busy=false; pendF=null; $('lstHead').classList.remove('thinking'); }
function render(){
  if (busy) return;
...
function shownOf(arr){ return arr; }
function renderList(arr){
  const shown = shownOf(arr);
  $('viewTitle').textContent = aiF ? (aiF.title || aiTitleFor(aiF.label)) : (cap ? 'Candidates' : 'All candidates for this position');
  syncChips();
  const ln = document.querySelector('.lst-note');
  if (ln) ln.hidden = !!aiF || !cap;
  ...
  $('cards').innerHTML = shown.map((c,n)=>`
    <article class="ccard${c.i===selI?' sel':''}" data-i="${c.i}" tabindex="0">
      ...
      ${c.note?`<div class="cc-note"><span class="avatar">${SHARER.init}</span><span><b>${SHARER.name.split(' ')[0]}:</b> \u201C${c.note}\u201D</span></div>`:''}
      ${barsHtml(c)}
      <div class="cc-state">
      ...
      <div class="cc-acts">
        ...
        <button class="ca hm" data-cact="hm" data-ci="${c.i}" title="Share candidates"><span class="material-icons-round">send</span><span class="lbl">Share</span><span class="lbl-s">Share</span></button>
      </div>
...
function railCrit(c){
  return COMP_DEFS.map(d=>{
    ...
    return `<div class="crit${open?' open':''}" data-k="${d.k}">
      ...
      <div class="crit-b"><div class="crit-bi">
        ${e.plus?`<div class="ev plus"><span class="material-icons-round pm">arrow_upward</span><span>${e.plus}</span><button class="ts" data-ts="${tsFor(c,d.k,'+')}"><span class="material-icons-round">play_arrow</span>${tsFor(c,d.k,'+')}</button></div>`:''}
        ${e.minus?`<div class="ev minus"><span class="material-icons-round pm">arrow_downward</span><span>${e.minus}</span><button class="ts" data-ts="${tsFor(c,d.k,'-')}"><span class="material-icons-round">play_arrow</span>${tsFor(c,d.k,'-')}</button></div>`:''}
      </div></div>
    </div>`;
  }).join('');
}
...
function hlSlides(c){
  ...
  const s = [];
  const ps = ev('ps');
  s.push({t:'Design challenge \u00b7 framing', ts:tsFor(c,'ps','+'), img:HL_SHOT.ps, pts:[`Unpacked the brief \u2014 probed users, constraints and success metrics before proposing.`, ps.plus, ps.minus].filter(Boolean).slice(0,3)});
  if (!c.caseNA){ const cr = ev('craft'); s.push({t:'Whiteboard \u00b7 design case', ts:null, img:HL_SHOT.craft, pts:[`Mapped flows and information architecture on the shared whiteboard.`, cr.plus, cr.minus].filter(Boolean).slice(0,3)}); }
  const cd = ev('coding');
  s.push({t:'Prototype exercise', ts:tsFor(c,'coding','+'), img:HL_SHOT.proto, pts:[cd.plus, `${c.passed} of ${c.total} interaction checks passed on the working prototype.`, cd.minus].filter(Boolean).slice(0,3)});
  const cm = ev('comm'), cl = ev('collab');
  s.push({t:'Design critique \u00b7 collaboration', ts:tsFor(c,'comm','+'), img:HL_SHOT.comm, pts:[cm.plus||cm.minus, cl.plus||cl.minus].filter(Boolean)});
  return s;
}
function hlHtml(c){
  const S = hlSlides(c);
  if (hlIdx >= S.length) hlIdx = 0;
  const sl = S[hlIdx];
  return `<div class="hl-card">
    <div class="hl-stage">
      <button class="hl-shot" data-hlopen="${hlIdx}" title="${sl.ts?'Play this moment':'View this slide'}">
        <img src="${sl.img}" alt="${sl.t} \u2014 interview recording" />
        <span class="hl-tag">${sl.t}</span>
        ${sl.ts?`<span class="hl-ts"><span class="material-icons-round">play_arrow</span>${sl.ts}</span>`:`<span class="hl-ts"><span class="material-icons-round">image</span>Still</span>`}
      </button>
      <button class="hl-arr prev" id="hlPrev" ${hlIdx===0?'disabled':''} title="Previous slide"><span class="material-icons-round">chevron_left</span></button>
      <button class="hl-arr next" id="hlNext" ${hlIdx===S.length-1?'disabled':''} title="Next slide"><span class="material-icons-round">chevron_right</span></button>
    </div>
    <ul class="hl-pts">${sl.pts.map(p=>`<li>${p}</li>`).join('')}</ul>
    <div class="hl-nav">
      <span class="hl-dots">${S.map((_,i)=>`<span class="hl-dot${i===hlIdx?' cur':''}" data-hldot="${i}"></span>`).join('')}</span>
    </div>
  </div>`;
}
function reportSec(c){
  return `<div class="rl-sec">
    <h3><span class="hl-grad">Interview highlights</span><button class="rep-dl" id="repDl"><span class="material-icons-round">download</span>Download</button></h3>
    <p class="rl-sub">Key moments from ${c.name.split(' ')[0]}&rsquo;s interview, with the recording/image timestamp for each</p>
    <div id="hlDeck">${hlHtml(c)}</div>
  </div>`;
}
function renderRail(){
  ...
  $('rail').innerHTML = `
    <div class="rail-scroll">
      ...
      ${c.note?`<div class="rl-note"><span class="avatar">${SHARER.init}</span><span><b>${SHARER.name.split(' ')[0]}:</b> \u201C${c.note}\u201D</span></div>`:''}
      <div class="rl-verdict">
        <div class="rv-k"><span class="rv-kl">AI Interview summary</span></div>
        <p class="rv-t">${v.txt}</p>
        <div class="rl-ask in-sum" id="askBox">
    ...
    <div class="rail-acts" id="railActs">
      ...
      <button class="ract hm" data-act="hm">Share</button>
    </div>`;
  queueClamp();
}

/* ===== actions ===== */
const VERB = {adv:'advanced', hm:'shared'};
/* share modal */
const PEOPLE = [
  {n:'Dana Whitmore', r:'Hiring manager \u00b7 Design', i:'DW', hm:true},
  {n:'Elliot Jon', r:'Design Director', i:'EJ'},
  {n:'Maya Patel', r:'Recruiting coordinator', i:'MP'},
  {n:'Jon Snow', r:'Product Lead', i:'JS'},
  {n:'Ravi Menon', r:'Staff Designer \u00b7 Interview panel', i:'RM'}
];
let shIds = [], shRec = [], shQ = '';
function shPaintSugg(){
  const q = shQ.trim().toLowerCase();
  const pool = PEOPLE.filter(p=>!shRec.includes(p) && (!q || (p.n+' '+p.r).toLowerCase().includes(q)));
  $('shSugg').innerHTML = pool.map(p=>`<div class="sg" data-add="${p.n}"><span class="avatar">${p.i}</span><span><b>${p.n}</b><span class="sr">${p.r}</span></span>${p.hm?'<span class="tag-hm">Hiring manager</span>':''}</div>`).join('')
    || `<div class="sg none">${q ? `No one named \u201C${shQ.trim()}\u201D on this position` : 'Everyone on this position has been added'}</div>`;
}
function shPaint(){
  $('shRecips').innerHTML = shRec.map((p,k)=>`<span class="rc" title="${p.r}"><span class="avatar">${p.i}</span>${p.n}<button class="rx" data-rx="${k}" title="Remove ${p.n}"><span class="material-icons-round">close</span></button></span>`).join('')
    + `<input id="shIn" type="text" placeholder="${shRec.length ? 'Add people\u2026' : 'Add a hiring manager or teammate\u2026'}" autocomplete="off" />`;
  shPaintSugg();
  $('shGo').disabled = !shRec.length;
  $('shSub').textContent = `${shIds.length===1 ? C[shIds[0]].name : shIds.length+' candidates'} \u00b7 ${$('posLbl').textContent}`;
  $('shCands').innerHTML = shIds.map(i=>`<span class="sh-c"><span class="avatar ${C[i].col}">${C[i].init}</span>${C[i].name}<small>${C[i].score.toFixed(1)}</small></span>`).join('');
  const hm = shRec.find(p=>p.hm) || shRec[0];
  $('shNote').placeholder = hm ? `Anything ${hm.n.split(' ')[0]} should know before reviewing\u2026` : 'Add a note for the reviewers\u2026';
}
function openShare(ids){
  if (!ids.length) return;
  shIds = ids; shRec = [PEOPLE[0]]; shQ = ''; $('shNote').value = '';
  shPaint(); $('shSugg').classList.remove('on');
  $('shModal').classList.add('on');
}
function closeShare(){ $('shModal').classList.remove('on'); $('shSugg').classList.remove('on'); }
function doShare(){
  if (!shRec.length) return;
  const ids = shIds, names = shRec.map(p=>p.n);
  const who = ids.length===1 ? C[ids[0]].name : `${ids.length} candidates`;
  const to = names.length===1 ? names[0] : names.length===2 ? names.join(' and ') : `${names[0]} and ${names.length-1} others`;
  closeShare(); act(ids, 'hm'); toast(`${who} shared with ${to}.`);
}
...
function toast(msg, noUndo){
  $('toastTxt').textContent = msg;
  $('toastUndo').hidden = !!noUndo;
...
document.addEventListener('click', e=>{
  if ($('shModal').classList.contains('on')){
    if (e.target.closest('#shX') || e.target.closest('#shCancel') || e.target === $('shModal')){ closeShare(); return; }
    if (e.target.closest('#shGo')){ doShare(); return; }
    const rx = e.target.closest('[data-rx]');
    if (rx){ shRec.splice(+rx.dataset.rx, 1); shPaint(); $('shIn').focus(); $('shSugg').classList.add('on'); return; }
    const ad = e.target.closest('[data-add]');
    if (ad){ const p = PEOPLE.find(x=>x.n===ad.dataset.add); if (p){ shRec.push(p); shQ=''; shPaint(); $('shIn').focus(); $('shSugg').classList.add('on'); } return; }
    if (e.target.closest('#shRecips')){ $('shIn').focus(); $('shSugg').classList.add('on'); return; }
    $('shSugg').classList.remove('on');
    return;
  }
  ...
  if (cact){ e.stopPropagation(); const ids = [+cact.dataset.ci]; if (cact.dataset.cact==='hm') openShare(ids); else act(ids, cact.dataset.cact); return; }
  if (e.target.closest('#repDl')){ toast(`Highlights deck for ${C[selI].name} downloaded as PDF.`, true); return; }
  if (e.target.closest('#hlPrev')){ hlIdx--; $('hlDeck').innerHTML = hlHtml(C[selI]); return; }
  if (e.target.closest('#hlNext')){ hlIdx++; $('hlDeck').innerHTML = hlHtml(C[selI]); return; }
  const hd = e.target.closest('[data-hldot]');
  if (hd){ hlIdx = +hd.dataset.hldot; $('hlDeck').innerHTML = hlHtml(C[selI]); return; }
  if (e.target.closest('#askX')){ clearTimeout(answer._t); clearInterval(answer._i); $('askA').classList.remove('on'); $('askBox').classList.remove('has-a'); $('askIn').value=''; queueClamp(); return; }
  ...
  if (ch){ const cr = ch.closest('.crit'), k = cr.dataset.k; if (openK.has(k)) openK.delete(k); else openK.add(k); cr.classList.toggle('open', openK.has(k)); queueClamp(); return; }
  const ho = e.target.closest('[data-hlopen]');
  if (ho){ openDeck(C[selI], +ho.dataset.hlopen); return; }
  if (e.target.closest('#reelPrev')){ deckI--; paintDeck(); return; }
  if (e.target.closest('#reelNext')){ deckI++; paintDeck(); return; }
  ...
  if (ba){ if (ba.dataset.bulk==='hm') openShare([...picked]); else act([...picked], ba.dataset.bulk); return; }
...
document.addEventListener('input', e=>{ if (e.target.id==='shIn'){ shQ = e.target.value; shPaintSugg(); $('shSugg').classList.add('on'); } });
document.addEventListener('keydown', e=>{
  if ($('shModal').classList.contains('on')){
    if (e.key==='Escape'){ closeShare(); return; }
    if (e.target.id==='shIn'){
      if (e.key==='Enter'){ e.preventDefault(); const f = $('shSugg').querySelector('[data-add]'); if (f) f.click(); return; }
      if (e.key==='Backspace' && !e.target.value && shRec.length){ shRec.pop(); shPaint(); $('shIn').focus(); $('shSugg').classList.add('on'); return; }
    }
    return;
  }
  ...
  if (e.key==='Escape'){
    ...
    else if (e.target.id==='askIn') e.target.blur();
    else if (selI!=null) setSel(null);
  }
  if ($('reelModal').classList.contains('on') && deckC){
    if (e.key==='ArrowRight' && deckI < hlSlides(deckC).length-1){ deckI++; paintDeck(); }
    if (e.key==='ArrowLeft' && deckI > 0){ deckI--; paintDeck(); }
  }
...
let deckC = null, deckI = 0;
function paintDeck(){
  const S = hlSlides(deckC), sl = S[deckI];
  $('reelTitle').textContent = `${deckC.name} \u00b7 interview highlights`;
  $('reelStage').innerHTML = `<img src="${sl.img}" alt="${sl.t}" />
    ${sl.ts?`<button class="vp-big" title="Play from ${sl.ts}"><span class="material-icons-round">play_arrow</span></button><span class="reel-still"><span class="material-icons-round">play_arrow</span>${sl.ts}</span>`
      :`<span class="reel-still"><span class="material-icons-round">image</span>Screen capture \u2014 no recording</span>`}
    <button class="reel-arr prev" id="reelPrev" ${deckI===0?'disabled':''} title="Previous"><span class="material-icons-round">chevron_left</span></button>
    <button class="reel-arr next" id="reelNext" ${deckI===S.length-1?'disabled':''} title="Next"><span class="material-icons-round">chevron_right</span></button>`;
  $('reelFoot').innerHTML = `<span class="reel-ft">${sl.t}</span>
    <ul class="reel-pts">${sl.pts.map(p=>`<li>${p}</li>`).join('')}</ul>
    <div class="reel-dots">${S.map((_,i)=>`<span class="hl-dot${i===deckI?' cur':''}" data-reeldot="${i}"></span>`).join('')}</div>`;
}
function openDeck(c, i){
  deckC = c; deckI = i || 0;
  paintDeck();
...
function openReel(c, ts){
  ...
  $('reelStage').innerHTML = `<img src="${c.photo}" alt="Highlight reel" /><button class="vp-big" title="Play"><span class="material-icons-round">play_arrow</span></button>`;
  $('reelFoot').innerHTML = '';
  deckC = null;
  $('reelModal').classList.add('on');
}
