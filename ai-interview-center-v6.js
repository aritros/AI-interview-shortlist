...
const FILTERS = {
  adv:      {label:'Like the ones you advanced', title:'Like the ones you advanced', f:c=>c.score>=4 && c.proctor,
  ...
  ps:       {label:'Strong problem solving', title:'Strongest problem solvers', f:c=>c.ps>=3.75,
  ...
  comm:     {label:'Great communicators', title:'Clearest communicators', f:c=>c.comm>=3.75,
  ...
  allTests: {label:'Passed all coding tests', title:'Passed every coding test', f:c=>c.passed===c.total,
  ...
  craft:    {label:'Completed the design case', title:'Completed the design case', f:c=>!c.caseNA,
  ...
  clean:    {label:'Clean proctoring only', title:'Clean proctoring sessions', f:c=>c.proctor,
  ...
  bar4:     {label:'Scoring 4.0+ overall', title:'Scoring 4.0 and above', f:c=>c.score>=4,
...
const SUGS = ['adv','ps','comm','allTests','clean','bar4'];
const CAP = 10;
let cap = CAP;
...
function chipsHtml(){
  ...
  if (aiF && aiF.k === '_free') out.push(chipOn(aiF.label, 'base', n(aiF.f)));
  out.push(cap
  ...
  SUGS.forEach(k=>out.push(aiF && aiF.k === k
...
function syncChips(force){
  const sig = (cap||'') + '|' + (aiF ? aiF.k+':'+aiF.label : '') + '|' + live().length;
...
document.addEventListener('click', e=>{
  ...
  if (chx){
    if (busy) return;
    if (chx.dataset.chipx === 'cap'){ cap = null; picked.clear(); render(); return; }
  ...
  if (e.target.closest('#clearF') || e.target.closest('#clearF2')){ stopFilter(); aiF=null; $('aiIn').value=''; setTyping(false); startPh(); render(); return; }
  const sug = e.target.closest('[data-q]');
  if (sug){ if (busy) return; const k = sug.dataset.q; cap = null; $('aiIn').value=''; setTyping(false); startPh(); runFilter({...FILTERS[k], k}); return; }
...
function submitFilter(){
  ...
  $('aiIn').value = ''; startPh();
  cap = null;
  runFilter(hit ? {...hit[1], k:hit[0]} : {k:'_free', label:raw, f:c=>c.score>=3.4});
}
