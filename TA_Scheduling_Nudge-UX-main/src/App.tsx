...
import AutomationSetupPanel from './components/AutomationSetupPanel';
import EnableAutomationModal from './components/EnableAutomationModal';
...
function App() {
  ...
  const [enableAutomationOpen, setEnableAutomationOpen] = useState(false);
  ...
  // "Automation nudge" is a standalone demo of just the automation modal —
  // it opens the moment that version is selected, and closes if you leave it.
  useEffect(() => {
    setEnableAutomationOpen(nudgeVersion === 'automation-nudge');
  }, [nudgeVersion]);
  ...
  const feedbackRowIndex = feedbackCandidate
    ? candidates.findIndex((c) => c.id === feedbackCandidate.id)
    : -1;
  const feedbackPendingCount = feedbackRowIndex >= 0 ? getFeedbackPending(feedbackRowIndex) : 0;
    ...
    nudgeVersion === 'inline-nudges'
      ...
      : nudgeVersion === 'inline-chat'
        ...
        : nudgeVersion === 'oda-2-0' || nudgeVersion === 'oda-3'
          ? 'oda20'
          : nudgeVersion === 'oda-4' || nudgeVersion === 'automation-nudge'
            ? 'none'
          : nudgeVersion === 'free-world'
            ? 'freeWorld'
    ...
    <div className={`app-shell${((nudgeVersion === 'oda-3' && nativeAssistantOpen) || (nudgeVersion === 'oda-4' && oda4PanelOpen)) ? ' app-shell--ai-push' : ''}`}>
      ...
      <div className="app-main">
        ...
        <CandidateTable
          ...
          onOpenFeedback={(c) => setFeedbackCandidate(c)}
        />
      </div>
      ...
      <AutomationSetupPanel open={panelOpen} onClose={() => setPanelOpen(false)} />
      <EnableAutomationModal
        open={enableAutomationOpen}
        onClose={() => setEnableAutomationOpen(false)}
      />
      <CandidateFeedbackPanel
        open={feedbackCandidate !== null}
        onClose={() => setFeedbackCandidate(null)}
        candidate={feedbackCandidate}
        candidates={candidates}
        onNavigate={setFeedbackCandidate}
      ...
      />
      {nudgeVersion !== 'oda-4' && nudgeVersion !== 'automation-nudge' && <TextSelectionPopover />}
