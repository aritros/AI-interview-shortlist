...
/* ===== state ===== */
let bucket=1, aiF=null, noteI=null, drawerI=null, openCmps=new Set();
const selected = new Set();
const BUCKETS = [
  {t:'All interviews', d:'Fresh interviews completed', cls:'c-all', f:()=>true},
...
function render(){
  ...
  renderBuckets(); renderFresh(); renderActed();
...
function renderFresh(){
  ...
  $('freshHead').innerHTML = `<div class="cb${allSel?' checked':''}" id="cbAll" title="Select all"></div>
    ...
    <div>Score</div><div class="fh-comp">Core competencies</div>`;
...
function renderActed(){
  ...
  $('hmRows').innerHTML = hms.map(c=>{
    ...
    return `<div class="lrow arow${c.i===drawerI?' sel':''}" data-i="${c.i}">
      <div class="avatar ${c.col}">${c.init}</div>
      <div class="ar-main">
        <div class="ar-l1"><span class="nm">${c.name}</span><span class="rl2">· ${c.role}</span><span class="sc2">${c.score.toFixed(1)}<small>/5</small></span>${noteBtn(c)}</div>
        <div class="ar-l2">
  ...
  $('advRows').innerHTML = advs.map(c=>`
    <div class="lrow arow${c.i===drawerI?' sel':''}" data-i="${c.i}">
      <div class="avatar ${c.col}">${c.init}</div>
      <div class="ar-main">
        <div class="ar-l1"><span class="nm">${c.name}</span><span class="rl2">· ${c.role}</span><span class="sc2">${c.score.toFixed(1)}<small>/5</small></span>${noteBtn(c)}</div>
        <div class="ar-l2"><span class="status-chip adv"><span class="material-icons-round">check</span>${c.stage}</span><span class="when">Advanced ${c.when}</span></div>
      </div>
...
function renderDrawer(){
  ...
  const advB = c.status==='adv' ? '' : `<button class="ract gy" data-adv="${c.i}">Advance stage<span class="material-icons-round">expand_more</span></button>`;
  ...
  <div class="dw-ai">
    <div class="ai-row">
      <div class="ai-main">
        ...
        <div class="vt" style="width:359px;height:63px">${v.txt}</div>
      </div>
...
document.addEventListener('click', e=>{
  ...
  if (e.target.closest('#histBtn')){ $('histOverlay').classList.add('open'); return; }
  if (e.target.closest('#histX') || e.target === $('histOverlay')){ $('histOverlay').classList.remove('open'); return; }
  ...
  if (row && row.dataset.i!=null){ hideStageMenu(); closeNotes(); $('histOverlay').classList.remove('open'); openDrawer(+row.dataset.i); return; }
...
document.addEventListener('keydown', e=>{ if(e.key==='Escape'){ hideStageMenu(); closeNotes(); closeDrawer(); $('fltFab').classList.remove('open'); closeReel(); $('histOverlay').classList.remove('open'); } });
